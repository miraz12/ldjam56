...
Texture::Texture(unsigned int textureIndex) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_width, m_height, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, m_missingTextureData);
...
unsigned int Texture::getWidth() { return m_width; }

unsigned int Texture::getHeight() { return m_height; }

void Texture::setTextureData(unsigned char *data, unsigned int width,
                             unsigned int height) {
  m_width = width;
  m_height = height;
  glBindTexture(GL_TEXTURE_2D, m_texture);
...
bool Texture::updateTextureSubData(unsigned char *data, unsigned int xOffset,
                                   unsigned int yOffset, unsigned int width,
                                   unsigned int height) {
