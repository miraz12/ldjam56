...
std::vector<Quad *> &InstancedQuadManager::getQuads() { return m_quads; }
...
void InstancedQuadManager::draw() {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, m_instanceVBO);
  glBufferData(GL_ARRAY_BUFFER, m_quadData.size() * sizeof(InstanceData),
               &m_quadData[0], GL_DYNAMIC_DRAW);
