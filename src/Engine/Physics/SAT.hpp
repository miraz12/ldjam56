...
##include "Shape.hpp"
  ...
  inline float getOverlap(const glm::vec2 &overlapVector,
    ...
    for (u32 i = 1; i < shapeAVertices.size(); i++) {
    ...
    for (u32 i = 1; i < shapeBVertices.size(); i++) {
  ...
  inline void calculateIntersectionPoint(
      ...
      const std::vector<glm::vec2> &shapeAVertices,
      const std::vector<glm::vec2> &shapeBVertices,
      const std::vector<u32> &shapeAIndices,
      const std::vector<u32> &shapeBIndices, glm::vec2 &intersectionPoint) {
    float maxA = glm::dot(intersectionLine, shapeAVertices[shapeAIndices[0]]);
    u32 maxAIndex = shapeAIndices[0];
    float minA = maxA;
    u32 minAIndex = maxAIndex;
    float maxB = glm::dot(intersectionLine, shapeBVertices[shapeBIndices[0]]);
    u32 maxBIndex = shapeBIndices[0];
    float minB = maxB;
    u32 minBIndex = maxBIndex;

    float tempDot = 0.0f;

    for (u32 i = 1; i < shapeAIndices.size(); i++) {
    ...
    for (u32 i = 1; i < shapeBIndices.size(); i++) {
    ...
    if (maxA > maxB && minA < minB) { // B interval is completely inside A
      ...
      intersectionPoint =
          (shapeBVertices[maxBIndex] + shapeBVertices[minBIndex]) * 0.5f;
               ...
               minB < minA) { // A interval is completely inside B
                              // interval, use middle of A interval
      intersectionPoint =
  ...
  inline bool getIntersection(Shape & shapeA, Shape & shapeB,
    ...
    for (u32 i = 0; i < shapeANormals.size(); i++) {
      bool reverse = false;
      float overlap =
    ...
    for (u32 i = 0; i < shapeBNormals.size(); i++) {
      bool reverse = false;
      float overlap =
  ...
  inline bool getIntersection(
      Shape & shapeA, Shape & shapeB, glm::vec2 & intersectionAxis,
    ...
    std::vector<u32> shapeAIntersectionPointIndices;
    std::vector<u32> shapeBIntersectionPointIndices;
    ...
    if (shapeAIntersectionPointIndices.size() == 1) {
      ...
      glm::vec2 intersectionLine = glm::vec2(
          intersectionAxis.y,
          -intersectionAxis.x); // Rotated intersection axis 90 degrees
      calculateIntersectionPoint(intersectionLine, shapeAVertices,
                                 shapeBVertices, shapeAIntersectionPointIndices,
  ...
  inline bool getRaycastOverlap(
      const glm::vec2 &testVec, const std::vector<glm::vec2> &vertices,
