...
Shape::Shape()
    : m_verticesNeedsUpdate(false), m_normalsNeedsUpdate(false),
...
const std::vector<glm::vec2> &Shape::getTransformedVertices() {
  if (m_verticesNeedsUpdate) {
    ...
    for (size_t i = 0; i < m_originalVertices.size(); i++) {
      m_transformedVertices.emplace_back(
          m_transformMatrix * glm::vec4(m_originalVertices[i], 0.0f, 1.0f));
    }
...
const std::vector<glm::vec2> &Shape::getTransformedNormals() {
  if (m_normalsNeedsUpdate) {
    ...
    for (size_t i = 0; i < m_originalNormals.size(); i++) {
      m_transformedNormals.emplace_back(
          glm::mat3(glm::transpose(glm::inverse(m_transformMatrix))) *
