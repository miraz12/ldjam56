...
InstancedShaderProgram::InstancedShaderProgram()
    : ShaderProgram("resources/Shaders/vertex_instanced.glsl",
                    "resources/Shaders/fragment_instanced.glsl") {
...
void InstancedShaderProgram::setupInstancedVertexAttributePointers() {
  ...
  for (unsigned int i = 0; i < 4; i++) {
    glVertexAttribPointer(6 + i, 4, GL_FLOAT, GL_FALSE, stride,
                          (void *)((16 + 4 * i) * sizeof(float)));
    glEnableVertexAttribArray(6 + i);
    glVertexAttribDivisor(6 + i, 1);
  }

  // Texture index
  glVertexAttribIPointer(10, 1, GL_INT, stride,
                         (void *)(totalFloats * sizeof(float)));
