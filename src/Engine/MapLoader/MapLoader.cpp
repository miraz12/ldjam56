...
bool MapLoader::allowMovement(int x, int y) {
  if (x < 0 || static_cast<unsigned int>(x) >= m_width || y < 0 ||
      static_cast<unsigned int>(y) >= m_height) {
    return false;
  }
...
unsigned int MapLoader::getWidth() { return m_width; }

unsigned int MapLoader::getHeight() { return m_height; }

void MapLoader::parseMap() {
  unsigned char *texData;
  texData = static_cast<unsigned char *>(
...
void MapLoader::loadMap(std::string mapName) {
  ...
  if (file.is_open()) {
    ...
    file >> width;
    file >> height;
    m_mapData = static_cast<unsigned int *>(
