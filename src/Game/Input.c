...
VV_LOCAL_SYMBOL  int compare_1156346975656988640_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
VV_LOCAL_SYMBOL void main__update_input(void);
...
int strings__levenshtein_distance(string a, string b) {
	Array_int f = array_repeat_to_depth(new_array_from_c_array(1, 1, sizeof(int), _MOV((int[1]){0})), (int)(b.len + 1), 0);
...
VV_LOCAL_SYMBOL array array_slice(array a, int start, int _end) {
	int end = (_end == 2147483647 ? (a.len) : (_end));
	#if !defined(CUSTOM_DEFINE_no_bounds_checking)
	{
		if (start > end) {
...
VNORETURN VV_LOCAL_SYMBOL void panic_debug(int line_no, string file, string mod, string fn_name, string s) {
	...
	#else
	{
		...
		eprintln( str_intp(2, _MOV((StrIntpData[]){{_SLIT("   v hash: "), /*115 &string*/0xfe10, {.d_s = _SLIT("dc6e317")}}, {_SLIT0, 0, { .d_c = 0 }}})));
		eprintln(_SLIT("========================================="));
		#if defined(_VNATIVE)
		{
			exit(1);
...
string string_replace_each(string s, Array_string vals) {
	...
	qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_1156346975656988640_RepIndex_by_idx);
	u8* b = malloc_noscan((int)(new_len + 1));
	int idx_pos = 0;
...
string string_trim(string s, string cutset) {
	...
	multi_return_int_int mr_40427 = string_trim_indexes(s, cutset);
	int left = mr_40427.arg0;
	int right = mr_40427.arg1;
