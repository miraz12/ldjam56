...
Game::Game(GLFWwindow *window) : m_ECSManager(&ECSManager::getInstance()) {
  ...
  gSys->getQuadManager()->getTexture(0).loadFromFile(
      "resources/Textures/instanced.png");
  gSys->getQuadManager()->getTexture(1).loadFromFile(
      "resources/Textures/HealthBar.png");
  gSys->getMapLoader()->getModelMatrix() = glm::translate(
      glm::mat4(1.0f),
      glm::vec3(0.5f * (float)gSys->getMapLoader()->getWidth(),
                0.5f * (float)gSys->getMapLoader()->getHeight(), 0.1f));
  gSys->getMapLoader()->getModelMatrix() =
      glm::scale(gSys->getMapLoader()->getModelMatrix(),
                 glm::vec3((float)gSys->getMapLoader()->getWidth(),
                           (float)gSys->getMapLoader()->getHeight(),
                           1.0f)); // Scale map to make the tile size 1x1
  gSys->getCamera()->setZoom(
      1.0f /
      (0.5f * (float)std::max(
                  gSys->getMapLoader()->getWidth(),
                  gSys->getMapLoader()->getHeight()))); // Zoom out so that the
...
void Game::init() {
  ...
  m_ECSManager->getGraphicsSystem()->getCamera()->setPosition(0.5f * 15.f,
                                                              0.5f * 15.f);
